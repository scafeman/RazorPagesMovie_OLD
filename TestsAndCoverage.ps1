...
$latestOpenCover = Join-Path -Path ((Get-ChildItem -Path $nugetOpenCoverPackage | Sort-Object Fullname -Descending)[0].FullName) -ChildPath "\.nuget\packages\opencover\4.7.922\tools\OpenCover.Console.exe"
...
$latestCoberturaConverter = Join-Path -Path (Get-ChildItem -Path $nugetCoberturaConverterPackage | Sort-Object Fullname -Descending)[0].FullName -ChildPath "\.nuget\packages\opencover\4.7.922\tools\OpenCoverToCoberturaConverter.exe"
